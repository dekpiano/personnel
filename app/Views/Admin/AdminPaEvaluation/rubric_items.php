                ...
                <?php if (!empty($rubrics)):
                    foreach ($rubrics as $item): ?>
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-warning btn-sm edit-rubric-btn"
                                    data-bs-toggle="modal" data-bs-target="#addRubricItemModal"
                                    data-id="<?= esc($item['ri_id']); ?>"
                                    data-part="<?= esc($item['ri_part']); ?>"
                                    data-domain="<?= esc($item['ri_domain']); ?>"
                                ...
                                <button type="button" class="btn btn-danger btn-sm delete-rubric-btn" data-id="<?= esc($item['ri_id']); ?>">ลบ</button>
                            </td>
                        </tr>
                    <?php endforeach;
                else: ?>
                    <tr>
                        <td colspan="6" class="text-center">ยังไม่มีหัวข้อการประเมิน</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="addRubricForm" action="<?= base_url('Admin/PaConfig/addRubricItem'); ?>" method="POST">
          ...
          <div class="row">
            <div class="mb-3">
            <label for="ri_academic_standing" class="form-label">สำหรับวิทยฐานะ (หากไม่เลือกจะถือว่าเป็นหัวข้อทั่วไป)</label>
            ...
            <div class="col-md-4 mb-3">
               <label for="ri_domain" class="form-label">หมวดหมู่ของงาน (เช่น ด้านการจัดการเรียนรู้)</label>
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control" id="ri_item_description" name="ri_item_description" rows="3" required></textarea>
          </div>
          <div class="mb-3">
            <label for="ri_expected_level_description" class="form-label">คำอธิบายระดับการปฏิบัติที่คาดหวัง</label>
            <textarea class="form-control" id="ri_expected_level_description" name="ri_expected_level_description" rows="3"></textarea>
          </div>
        </form>
      </div>
...
$(document).ready(function() {
    ...
    function resetRubricForm() {
        ...
        $('#addRubricForm input, #addRubricForm textarea, #ri_academic_standing').removeClass('is-invalid');
    }

    // When the "Add New Rubric Item" button is clicked
    $('button[data-bs-target="#addRubricItemModal"]').on('click', function() {
        resetRubricForm();
    });
    ...
    $('.edit-rubric-btn').on('click', function() {
        ...
        $('#addRubricForm').attr('action', '<?= base_url('Admin/PaConfig/updateRubricItem'); ?>'); // Assuming an update endpoint
    ...
    $('#addRubricForm input, #addRubricForm textarea, #ri_academic_standing').on('change input', function() {
        if ($(this).val().trim() !== '') {
