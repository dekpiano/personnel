...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('attendance-form').addEventListener('submit', function (e) {
        ...
        fetch('../Admin/SaveAttendance/DB/Select/SaveAttendanceToDB', { method: 'POST', body: formData })
            .then(res => {
            ...
            .then(result => {
                ...
                if (result.success) {
                    Swal.fire({ icon: 'success', title: 'บันทึกสำเร็จ!', text: 'บันทึกข้อมูลการมาทำงานเรียบร้อยแล้ว', timer: 1500, showConfirmButton: false });
            ...
            .finally(() => {
                if (btn) {
                    btn.disabled = false;
                    btn.innerHTML = '<i class="bi bi-check2-circle me-1"></i> บันทึกข้อมูล';
                }
            });
    });
    ...
    function updateDashboard() {
        ...
        fetch('../Admin/SaveAttendance/DB/Select/DashboardAttendance?' + params)
            ...
            .then(data => {
                // Chart
                const series = [data.stats.present, data.stats.late, data.stats.absent, data.stats.personal, data.stats.sick, data.stats.official, data.stats.other];
                const labels = ['มาทำงาน', 'มาสาย', 'ขาดงาน', 'ลากิจ', 'ลาป่วย', 'ไปราชการ', 'อื่นๆ'];
                const colors = ['#28a745', '#17a2b8', '#dc3545', '#fd7e14', '#ffc107', '#007bff', '#6c757d'];

                if (chart) chart.destroy();
                chart = new ApexCharts(document.querySelector("#chart"), {
                    chart: { type: 'donut', height: 350, fontFamily: 'thsarabun, sans-serif' },
                    ...
                    plotOptions: {
                        pie: {
                            donut: {
                                ...
                                labels: {
                                    show: true,
                                    total: { show: true, label: 'มาทำงาน', formatter: () => data.stats.present_percent + '%' }
                                }
                            }
                        }
                    },
    ...
    function getBadgeClass(status) {
        switch(status) {
            case 'มา': return 'badge-present';
            case 'สาย': return 'badge-late';
            case 'ขาด': return 'badge-absent';
            case 'ลาป่วย': return 'badge-sick';
            case 'ลากิจ': return 'badge-personal';
            case 'ไปราชการ': return 'badge-official';
            default: return 'badge-other';
        }
    }

    const initialDateInput = document.getElementById('dateInput');
    if(initialDateInput) initialDateInput.addEventListener('change', updateDashboard);
    updateDashboard();
});
