...
function calculateAge(dateStr) {
  if (!dateStr || dateStr === '00/00/0000' || dateStr === '0000-00-00') return '';
...
function getFormDataWithCroppedImages(form) {
  ...
  $(form).find('input[type="file"].crop-target').each(function() {
  ...
  $.ajax({
    ...
    success: function (res) {
      if (res.status === "success" || res == 1) {
        ...
        Swal.fire("แจ้งเตือน!", res.message || "บันทึกข้อมูลไม่สำเร็จ!", "error");
      }
    },
  });
});
    ...
    $.ajax({
      ...
      success: function (res) {
        if (res.status === "success" || res == 1) {
          ...
          Swal.fire("แจ้งเตือน!", "เปลี่ยนรูปภาพไม่สำเร็จ: " + (res.message || res), "error");
        }
      },
      error: function (xhr, status, error) {
        console.error("Upload Error:", xhr.responseText);
        Swal.fire("เกิดข้อผิดพลาด!", "ไม่สามารถอัปโหลดรูปภาพได้ (500 Internal Server Error)<br><small>" + error + "</small>", "error");
      }
    });
  } else {
      ...
      $(".subdistrict")
        ...
        if (data[0].education && data[0].education.length > 0) {
            data[0].education.forEach(edu => {
                const newRow = `
                    <tr data-id="${edu.id}">
                    <td>${edu.edu_level}</td>
                    <td>${edu.edu_degree} ${edu.edu_major ? `(${edu.edu_major})` : ''}</td>
                    <td>${edu.edu_institute}</td>
                    <td>${edu.edu_year}</td>
                    <td class="text-center">
                        <button type="button" class="btn btn-sm btn-icon btn-label-danger remove-education" data-id="${edu.id}">
                        <i class="bx bx-trash"></i>
                        </button>
                    </td>
                    </tr>
        ...
        if (data[0].work_history && data[0].work_history.length > 0) {
            data[0].work_history.forEach(work => {
                const salary = work.work_salary > 0 
                  ? parseFloat(work.work_salary).toLocaleString('th-TH', {minimumFractionDigits: 2}) 
                  : '-';
                ...
                const newRow = `
                    ...
                    <td>
                        ...
                        <div class="text-muted small">${work.work_level || ''}</div>
                    </td>
                    ...
                    <td class="text-center">
                        <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-sm btn-icon btn-label-danger remove-work-history" data-id="${work.id}">
                                <i class="bx bx-trash"></i>
                            </button>
                        </div>
                    </td>
        ...
        if (data[0].decorations && data[0].decorations.length > 0) {
            data[0].decorations.forEach(deco => {
                ...
                const gazetteInfo = `เล่ม ${deco.deco_gazette_vol || '-'} ตอน ${deco.deco_gazette_part || '-'} หน้า ${deco.deco_gazette_page || '-'} ลำดับ ${deco.deco_gazette_seq || '-'}`;
        ...
        if (data[0].training && data[0].training.length > 0) {
            data[0].training.forEach(train => {
                const dateRange = train.train_end_display && train.train_end_display !== '-' 
                ...
                const newRow = `
                    <tr data-id="${train.id}">
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-sm btn-icon btn-label-danger remove-training" data-id="${train.id}">
                                <i class="bx bx-trash"></i>
                            </button>
                        </td>
                    </tr>
        ...
        leaveTable.empty();
        if (data[0].leave_history && data[0].leave_history.length > 0) {
            data[0].leave_history.forEach(leave => {
                const dateRange = leave.leave_end_display && leave.leave_end_display !== '-' 
                    ? `${leave.leave_start_display} - ${leave.leave_end_display}`
                    : leave.leave_start_display;
                
                const newRow = `
                    <tr data-id="${leave.id}">
                        <td>${dateRange}</td>
                        <td><div class="fw-bold">${leave.leave_type}</div></td>
                        <td class="text-center">${leave.leave_days || '-'}</td>
                        <td><small>${leave.leave_note || '-'}</small></td>
                        <td class="text-center">
                            <button type="button" class="btn btn-sm btn-icon btn-label-danger remove-leave" data-id="${leave.id}">
                                <i class="bx bx-trash"></i>
                            </button>
                        </td>
                    </tr>
    ...
    $.ajax({
        ...
        success: function(data) {
            const populateSelect = ($select, options) => {
                $select.empty().append('<option value="" selected disabled>เลือกหรือพิมพ์เพิ่ม...</option>');
                options.forEach(opt => {
      ...
      success: function(res) {
          ...
          if(res.status === 'success') {
              const newRow = `
                <tr data-id="${res.id}">
                  <td>${level}</td>
                  <td>${degree} ${$("#edu_major").val() ? `(${$("#edu_major").val()})` : ''}</td>
                  <td>${institute}</td>
                  <td>${year}</td>
                  <td class="text-center">
                    <button type="button" class="btn btn-sm btn-icon btn-label-danger remove-education" data-id="${res.id}">
                      <i class="bx bx-trash"></i>
                    </button>
                  </td>
                </tr>
...
$("#FormLicenseInfo").off("submit").on("submit", function(e){
    e.preventDefault();
    e.stopImmediatePropagation();
    const pers_id = new URLSearchParams(window.location.search).get("pers_id") || $("#pers_id_hidden").val() || $("input[name='pers_id']").val();
    ...
    $.ajax({
        ...
        success: function(res) {
            Swal.close();
            if(res.status === 'success') {
                Swal.fire("สำเร็จ", "บันทึกข้อมูลใบประกอบวิชาชีพเรียบร้อย", "success");
            } else {
                Swal.fire("ล้มเหลว", "บันทึกข้อมูลไม่สำเร็จ", "error");
            }
        },
...
$("#workHistoryForm").off("submit").on("submit", function(e){
    e.preventDefault();
    const pers_id = new URLSearchParams(window.location.search).get("pers_id") || $("#pers_id_hidden").val() || $("input[name='pers_id']").val();
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if(res.status === 'success') {
                ...
                bootstrap.Modal.getInstance(document.getElementById('workHistoryModal')).hide();
...
$(document).on("click", ".edit-work-history", function() {
    ...
    if (data.work_command_date) {
         ...
         if(!isNaN(d.getTime())) {
             ...
             const thDate = `${d.getDate().toString().padStart(2,'0')}/${(d.getMonth()+1).toString().padStart(2,'0')}/${thYear}`;
      ...
      $.ajax({
        ...
        success: function (res) {
          if (res.status === "success") {
            ...
            Swal.fire("ข้อผิดพลาด!", res.message || "ไม่สามารถล้างไฟล์ขยะได้", "error");
          }
        },
        error: function () {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data && data[0]) {
                ...
                // Header Info
                $("#preview_pers_img").attr("src", baseUrl + "/uploads/admin/Personnal/" + (p.pers_img || 'default.png'));
                $("#preview_fullname").text(p.pers_prefix + p.pers_firstname + " " + p.pers_lastname);
                $("#preview_pers_id").text(p.pers_id);
                $("#preview_posi_name").text(p.posi_name || '-');
                $("#preview_full_edit_link").attr("href", baseUrl + "/Admin/WorkPerson/Personnel/Update/" + p.pers_id);
                
                // General Info
                $("#preview_birthday").text(p.pers_britday || '-');
                $("#preview_id_card").text(p.pers_id_card || '-');
                $("#preview_nat_race").text((p.pers_nationality || '-') + " / " + (p.pers_race || '-'));
                ...
                $("#preview_license").text(p.pers_license_no ? "มี (เลขที่ " + p.pers_license_no + ")" : "ไม่มีข้อมูล");

                // Education
                const eduBody = $("#preview_edu_table tbody").empty();
                if (p.education && p.education.length > 0) {
                    p.education.forEach(e => {
                        eduBody.append(`<tr><td>${e.edu_year || '-'}</td><td>${e.edu_degree || '-'}</td><td>${e.edu_major || '-'}</td><td>${e.edu_institution || '-'}</td></tr>`);
                    });
                } else {
                    eduBody.append('<tr><td colspan="4" class="text-center text-muted">ไม่มีข้อมูล</td></tr>');
                }
                ...
                if (p.work_history && p.work_history.length > 0) {
                    p.work_history.forEach(w => {
                        workBody.append(`<tr><td>${w.work_date_display || '-'}</td><td>${w.work_position || '-'}</td><td>${w.work_academic || '-'}</td><td>${w.work_department || '-'}</td></tr>`);
                    });
                } else {
                    workBody.append('<tr><td colspan="4" class="text-center text-muted">ไม่มีข้อมูล</td></tr>');
                }
                ...
                if (p.decorations && p.decorations.length > 0) {
                    p.decorations.forEach(d => {
                        decoBody.append(`<tr><td>${d.deco_year || '-'}</td><td>${d.deco_badge || '-'}</td><td>เล่มที่ ${d.deco_gazette_volume || '-'} ตอนที่ ${d.deco_gazette_section || '-'}</td></tr>`);
                    });
                } else {
                    decoBody.append('<tr><td colspan="3" class="text-center text-muted">ไม่มีข้อมูล</td></tr>');
                }
                ...
                if (p.training && p.training.length > 0) {
                    p.training.forEach(t => {
                        const date = t.train_end_display && t.train_end_display !== '-' ? t.train_start_display + " - " + t.train_end_display : t.train_start_display;
                        trainBody.append(`<tr><td>${date}</td><td>${t.train_name || '-'}</td><td>${t.train_location || '-'}</td><td class="text-center">${t.train_hours || '-'}</td></tr>`);
                    });
                } else {
                    trainBody.append('<tr><td colspan="4" class="text-center text-muted">ไม่มีข้อมูล</td></tr>');
                }
